...
class MultiLevelAtomRTE(BaseRTE):
    ...
    def calculate_eta_rho_a(self, angles: Angles, rho: Rho, atmosphere_parameters: AtmosphereParameters) -> np.ndarray:
        r"""
        Calculate :math:`\eta_A + i \rho_A` for all Stokes components: complex ``[4, len(nu)]`` for
        I, Q, U, V. The rho-independent operator is compiled once per (angles, atmosphere) and cached;
        only the multiplication by rho and the ``N`` prefactor vary between calls.

        Reference: (LL04 7.15 ac)
        """
        operator = self.eta_rho_a_cache.get(angles, atmosphere_parameters)
        if operator is None:
            frame = Frame.from_sum_limits(base_frame=self.create_base_frame(), sum_limits=self.AFrameSumLimits())
            ...
            frame.register_multiplication(
                ...
                phi=lambda level_upper_id, Mu, level_lower_id, Ml, gu, gl: self.phi(
                    level_upper_id=level_upper_id, Mu=Mu, level_lower_id=level_lower_id,
                    Ml=Ml, gu=gu, gl=gl, atmosphere_parameters=atmosphere_parameters,
                ),
                elementwise=True,
            )  # fmt: skip
            # Reduce every column except the rho-index columns (Kl, Ql, level_lower_id), heavy K, Q first.
            frame.reduce_partially(
                "K", "Q", "Ju", "Jl",
                "transition_id", "level_upper_id", "gl", "gu", "Ml", "Mʹl", "Mu", "q", "qʹ",
            )  # fmt: skip
            operator = frame.to_operator(ordered_multiplicand_keys=["Kl", "Ql", "level_lower_id"])
    ...
    def calculate_eta_rho_s(self, angles: Angles, rho: Rho, atmosphere_parameters: AtmosphereParameters) -> np.ndarray:
        r"""
        Calculate :math:`\eta_S + i \rho_S` for all Stokes components: complex ``[4, len(nu)]`` for
        I, Q, U, V. The rho-independent operator is compiled once per (angles, atmosphere) and cached;
        only the multiplication by rho and the ``N`` prefactor vary between calls.
        ...
        if operator is None:
            frame = Frame.from_sum_limits(base_frame=self.create_base_frame(), sum_limits=self.SFrameSumLimits())
            frame.register_multiplication(
            ...
            # Reduce every column except the rho-index columns (Ku, Qu, level_upper_id), heavy K, Q first.
            frame.reduce_partially(
                "K", "Q", "Jl", "Ju",
                "transition_id", "level_lower_id", "gl", "gu", "Mu", "Mʹu", "Ml", "q", "qʹ",
            )  # fmt: skip
            operator = frame.to_operator(ordered_multiplicand_keys=["Ku", "Qu", "level_upper_id"])
