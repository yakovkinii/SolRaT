...
class TestStatisticalEquilibriumEquations(unittest.TestCase):
    def test_statistical_equilibrium_equations_resonance(self):
        ...
        atom.fill_all_equations(
            atmosphere_parameters=atmosphere_parameters,
            radiation_tensor_in_magnetic_frame=radiation_tensor,
        )
        solution_legacy = atom_legacy.get_solution_direct()
        solution = atom.get_solution()
        ...
        assert (
            abs(
                rho_l_0_0
                - solution_legacy(term_id=level_registry.get_term(beta="1s", L=0, S=0).term_id, K=0, Q=0, J=0, Jʹ=0)
            )
            < 1e-15
        ).all()
        assert (
            abs(
                rho_u_0_0
                - solution_legacy(term_id=level_registry.get_term(beta="2p", L=1, S=0).term_id, K=0, Q=0, J=1, Jʹ=1)
            )
            < 1e-15
        ).all()
        assert (
            abs(rho_l_0_0 - solution(term_id=level_registry.get_term(beta="1s", L=0, S=0).term_id, K=0, Q=0, J=0, Jʹ=0))
            < 1e-15
        ).all()
        assert (
            abs(rho_u_0_0 - solution(term_id=level_registry.get_term(beta="2p", L=1, S=0).term_id, K=0, Q=0, J=1, Jʹ=1))
            < 1e-15
        ).all()
