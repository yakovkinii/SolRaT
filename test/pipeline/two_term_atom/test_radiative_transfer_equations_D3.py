...
class TestRadiativeTransferEquationsD3(unittest.TestCase):
    def test_radiative_transfer_equations_d3(self):
        ...
        lambda_A = np.arange(5877, 5878, 0.01)
        ...
        #
        # plt.plot(lambda_A, eta_sI, label=r"$\eta_s I$")
        # plt.plot(lambda_A, eta_sV, "-.", label=r"$\eta_s V$")
        plt.plot(lambda_A, eta_s_analytic, "--", label=r"$\eta_s$ (analytic test case, no magnetic field)")
        # plt.plot(np.arange(5.5e14, 7e14, 2e12), eta_s_values)
        plt.xlabel(r"$\lambda$ \AA")
        plt.ylabel(r"$\eta_s$")
        plt.title(r"$\eta_s$ vs $\lambda$")
        plt.legend()
        plt.show()
